...
inline USBMSC usbMsc;

inline bool mscStartStop(uint8_t, bool start, bool load_eject) {
    if (load_eject) {
        if (start) {
            FFat.end();
        } else {
...
inline int32_t mscRead(uint32_t lba, uint32_t offset, void* buffer, uint32_t bufsize) {
    uint32_t addr = lba * SECTOR_SIZE + offset;
    if (esp_partition_read(ffatPartition, addr, buffer, bufsize) != ESP_OK) return -1;
    return bufsize;
}

inline int32_t mscWrite(uint32_t lba, uint32_t offset, uint8_t* buffer, uint32_t bufsize) {
    uint32_t addr = lba * SECTOR_SIZE + offset;
